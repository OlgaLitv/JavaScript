...
const prod = document.getElementsByClassName("products")[0];
...
function showCategory(category) {
    let list_prod = products[category];
    for(let i=0; i < list_prod.length; i++){
        prod.insertAdjacentHTML("afterbegin", getProductMarkup(list_prod[i]));
    }
    //prod.insertAdjacentHTML("afterbegin", strng);
}
...
function getProductMarkup(product) {
    return `<div class="product"><div>${product.name}</div>
    <img src="${product.imageUrl}" alt=""><div>${product.price}</div>
    <a href="https://example.com/products/${product.id}">Подробнее</a></div>`;
